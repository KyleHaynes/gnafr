  ...
  server <- function(input, output, session) {
    ...
    output$results_table <- reactable::renderReactable({
      ...
      reactable::reactable(
        ...
        striped = TRUE,
        resizable = TRUE,
        defaultSorted = list(matched = "desc", input_id = "asc", match_rank = "asc"),
        columns = list(
          ...
          match_basis = reactable::colDef(name = "Match basis", minWidth = 140),
...
#'
#' Compares each input string with its matched address label (both passed
#' through the same text normalisation) and appends four 0-100 similarity
...
#' By default the source is `input_raw`, not the reconstructed parsed address.
#' Normalisation repairs encoding and spacing, uppercases, and removes commas
#' and full stops. It does not expand street-type abbreviations or reconstruct
#' parsed components.
#' These scores do not participate in candidate retrieval or ranking.
#'
#' @param x A `data.table` returned by [gnaf_match()].
#' @param input Which input text to compare: `"raw"` (default) or
#'   `"standardised"` (the reconstructed `input_standardised` column).
...
#' }
#' @export
gnaf_text_scores <- function(x, input = c("raw", "standardised")) {
  if (!is.data.table(x)) stop("'x' must be a data.table returned by gnaf_match()")
  input <- match.arg(input)
  input_col <- paste0("input_", input)
  missing_cols <- setdiff(c(input_col, "address_label", "matched"), names(x))
  if (length(missing_cols) > 0L) {
    stop("'x' is missing columns: ", paste(missing_cols, collapse = ", "))
  }

  out <- copy(x)
  input_norm <- .normalize_addr(out[[input_col]])
  ...
  matched_idx <- out$matched %in% TRUE & !is.na(input_norm) &
    nzchar(input_norm) & !is.na(match_norm) & nzchar(match_norm)
